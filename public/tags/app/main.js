...
riot.tag2('game-list', '<div class="container"> <div class="row"> <div class="col-xs-12"> <form> <input type="text" id="search_key" placeholder="検索ワードを入力" ref="search_key"> </form> </div> <div class="col-xs-12"> <table class="table table-hover"> <thead> <tr> <th onclick="{_func.sortGameList}" name="name" ref="name" class="list-pointer"> Game Name <span class="{status.sortIcon[0]}" aria-hidden="true" aria-label="Right Align"></span> </th> <th onclick="{_func.sortGameList}" name="version" ref="version" class="list-pointer"> Version <span class="{status.sortIcon[1]}" aria-hidden="true" aria-label="Right Align"></span> </th> <th ref="edit_delete"></th> </tr> </thead> <tbody> <tr each="{item, i in status.getData()}" name="{item.publicId}" if="{status.pageLabelCheck(status.pageIdLabel[i].label)}" class="list-pointer"> <th onclick="{_func.viewFileList}" name="{item.publicId}">{item.name}</th> <th onclick="{_func.viewFileList}" name="{item.publicId}">{item.version}</th> <th> <button class="btn btn-primary" id="admin_{item.publicId}_{item.name}_{item.version}" onclick="{_func.editGame}"> <span class="glyphicon glyphicon-wrench" aria-hidden="true" aria-label="Right Align"></span> 編集 </button> <button class="btn btn-primary" name="{item.publicId}" onclick="{_func.deleteGame}"> <span class="glyphicon glyphicon-trash" aria-hidden="true" aria-label="Right Align"></span> 削除 </button> </th> </tr> </tbody> </table> </div> <div class="col-xs-12"> <ul class="pagination"> <li name="previous" onclick="{_func.pageNation}"> <a href="#" aria-label="Previous" name="previous"> <span aria-hidden="true" name="previous"> &laquo; </span> </a> </li> <li each="{page,i in status.getPage()}" name="{page.label}" class="{page.pageClass} " onclick="{_func.pageNation}"> <a href="#" name="{page.label}"> <span aria-hidden="true" name="{page.label}"> {page.label} </span> </a> </li> <li name="next" class="pageL1" onclick="{_func.pageNation}"> <a href="#" aria-label="Next" name="next"> <span aria-hidden="true" name="next"> &raquo; </span> </a> </li> </ul> </div> <div if="{!status.edit && !status.newCreate}" class="col-xs-12"> <button class="btn btn-primary" onclick="{status.newCreateOn}"> <span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 新規作成 </button> </div> <div if="{!status.edit && status.newCreate}" class="col-xs-12"> <form class="form-horizontal" onsubmit="{_func.saveGame}"> <div class="form-group"> <label class="control-label col-xs-3">Game Title</label> <div class="col-xs-9"> <input type="text" class="form-control" name="title" placeholder="Game Title"> </div> </div> <div class="form-group"> <label class="control-label col-xs-3">Version</label> <div class="col-xs-9"> <input type="text" class="form-control" name="version" placeholder="v1.4"> </div> </div> <div class="col-xs-12 col-xs-offset-3"> <button type="submit" class="btn btn-primary">新規保存</button> <button class="btn btn-primary" onclick="{status.newCreateOff}">キャンセル</button> </div> </form> </div> <div if="{status.edit && !status.newCreate}" class="col-xs-12"> <form class="form-horizontal" onsubmit="{_func.adminGame}"> <input type="hidden" name="publicId" riot-value="{status.publicGameId}"> <div class="form-group"> <label class="control-label col-xs-3">Game Title</label> <div class="col-xs-9"> <input type="text" class="form-control" name="title" placeholder="{status.editName}"> </div> </div> <div class="form-group"> <label class="control-label col-xs-3">Version</label> <div class="col-xs-9"> <input type="text" class="form-control" name="version" placeholder="{status.editVersion}"> </div> </div> <div class="col-xs-12 col-xs-offset-3"> <button type="submit" class="btn btn-primary"> <span class="glyphicon glyphicon-save" aria-hidden="true"></span> 保存 </button> <button class="btn btn-primary" onclick="{_func.cancel}"> <span class="glyphicon glyphicon-remove" aria-hidden="true"></span> キャンセル </button> </div> </form> </div> </div> </div>', 'game-list .list-pointer,[data-is="game-list"] .list-pointer{ cursor: pointer; }', '', function(opts) {
        var self = this;
        this.mixin(OptsMixin);
...
riot.tag2('key-list', '<div id="key-list-table"> <table id="key-table" class="table table-hover"> <thead> <tr> <th onclick="{_func.sortKeyList}" id="sort_{headingName[0]}" ref="{headingName[0]}">File Name </th> </tr> </thead> <tbody> <tr each="{item, i in key_data}" id="{item.publicId}"> <th onclick="{_func.viewDocumentEditor}" id="name_{item.publicId}">{item.name}</th> </tr> </tbody> </table> </div>', '', '', function(opts) {
        var self = this,
...
riot.tag2('user-mypage', '<div class="container"> <nav class="nav nav-tabs" id="myPageTab" role="tablist"> <a class="nav-item nav-link active" id="nav-home-tab" data-toggle="tab" href="#nav-home" role="tab" aria-controls="nav-home" aria-selected="true"> ユーザー情報 </a> <a class="nav-item nav-link" id="nav-profile-tab" data-toggle="tab" href="#nav-profile" role="tab" aria-controls="nav-profile" aria-selected="false"> メッセージ </a> </nav> <div class="tab-content" id="nav-tabContent"> <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab"> <br> <user-mypage-admin user="{opts.user}"></user-mypage-admin> </div> <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab"> dddddddddddd... </div> </div> </div>', '', '', function(opts) {
        var self = this;
        this.mixin(OptsMixin);
